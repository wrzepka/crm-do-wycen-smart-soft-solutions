...
import { MoreHorizontal, FileText, Pencil, Trash2, Mail, History, Loader2 } from 'lucide-react';
...
type QuoteWithRelations = {
  ...
  total_cost: number | string; // need to calc margin
  client: {
    first_name: string;
    last_name: string;
    email: string;
...
const statusMap: Record<string, { label: string; variant: BadgeVariant }> = {
  ...
  ACCEPTED: { label: 'Zaakceptowana', variant: 'default' }, // 'success' nie istnieje w standardowym shadcn, zmieniono na default + klasa
      ...
      <Table>
        <TableHeader className="bg-slate-50 dark:bg-slate-900/50">
          <TableRow className="border-slate-200 dark:border-slate-800 hover:bg-transparent">
            ...
            {/* Client- no static width */}
            <TableHead className="text-slate-500 font-semibold">Klient</TableHead>
            <TableHead className="text-right text-slate-500 font-semibold w-[150px]">
              Wartość (Netto)
            </TableHead>
            {/* NEW MARGIN COLUMN */}
            <TableHead className="text-right text-slate-500 font-semibold w-[120px]">
              Marża
            </TableHead>
        ...
        <TableBody>
          {data.length === 0 ? (
            ...
            data.map((quote) => {
              ...
              const statusConfig = statusMap[quote.status] || {
                label: quote.status,
                variant: 'outline',
              };

              // calculating margin and margin percentage
              const totalNet = Number(quote.total_net);
              const totalCost = Number(quote.total_cost || 0); // null safety
              ...
              return (
                ...
                >
                  <TableCell className="font-medium font-mono pl-6">
                    <Link
                      href={`/dashboard/quotes/${quote.id}/edit`}
                      className="hover:underline text-primary dark:text-blue-400 text-sm"
                    >
                  ...
                  <TableCell>
                    <div
                      className={`flex flex-col text-sm ${isExpired ? 'text-red-600 font-medium' : 'text-slate-500 dark:text-slate-400'}`}
                    >
                  ...
                  <TableCell>
                    <Badge
                      ...
                      className={`text-[10px] px-2 py-0.5 ${quote.status === 'ACCEPTED' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}`}
                    >
                      {statusConfig.label}
                    </Badge>
                  </TableCell>
