...
export function QuoteVersionSelector({
  ...
  const handleVersionChange = (value: string) => {
    // Redirect to edit selected version
  ...
  // If new quote (no code) or no versions, do not display
    ...
    <div className="flex flex-col gap-3 p-4 mb-6 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-800">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="outline" className="text-[10px]">
          v{versions.find((v) => v.id === currentId)?.version || '?'}
        </Badge>
      </div>
              ...
              <div className="flex items-center justify-between w-full gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {/* Using quote_date according to your model */}
