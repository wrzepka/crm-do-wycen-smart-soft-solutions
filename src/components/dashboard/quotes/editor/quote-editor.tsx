...
import { QuoteVersionSelector, QuoteVersion } from './quote-version-selector';
import { Separator } from '@/components/ui/separator';
import { useMemo } from 'react';
import { CreatePricingHistoryInput, UpdatePricingHistoryInput } from '@/types/pricing';

// --- HELPER TYPES (Exported for page.tsx) ---
...
export interface InitialService {
  ...
  total_net?: number | string;
  total_cost?: number | string;
  serviceResources: InitialResource[];
}
...
export interface InitialData {
  ...
  total_cost?: number | string;
  pricingServices: InitialService[];
}
...
export function QuoteEditor({
  ...
  const defaultValues: QuoteFormValues = useMemo(() => {
    if (initialData) {
      return {
        ...
        services:
          initialData.pricingServices?.map((s) => ({
            id: s.id,
            name: s.name,
  ...
  const methods = useForm<QuoteFormValues>({
    ...
    resolver: zodResolver(
      createPricingHistoryWithServicesSchema.passthrough(),
    ) as unknown as Resolver<QuoteFormValues>,
    defaultValues,
  });
  ...
  return (
    <FormProvider {...methods}>
      <form
        ...
        className="flex h-full flex-col lg:flex-row bg-slate-50/50 dark:bg-[#020817] overflow-hidden"
      >
        {/* LEFT COLUMN */}
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 border-r border-slate-200 dark:border-slate-800 h-full">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {isEditMode
                    ...
                    : 'Wypełnij dane projektu i zakres prac.'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8 max-w-2xl">
            <QuoteContextSection clients={clients} />
            <Separator />
            <div>
          ...
          <div className="border-t bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm p-4 flex items-center justify-between shrink-0 z-10">
            <div className="text-sm text-slate-500">
              ...
              <span className="font-medium text-slate-900">
                {isEditMode ? initialData.status : 'Nowa'}
              </span>
            </div>
