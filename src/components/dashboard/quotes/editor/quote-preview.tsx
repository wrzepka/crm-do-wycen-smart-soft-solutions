...
export function QuotePreview({ clients }: QuotePreviewProps) {
  ...
  // State for expanding/collapsing service list
  ...
  const clientName = client ? `${client.first_name} ${client.last_name}` : 'Nie wybrano klienta';
  ...
  services.forEach((service) => {
    ...
    const serviceRevenue = resources.reduce(
      (acc: number, res) => acc + Number(res.quantity || 0) * Number(res.unit_price || 0),
      0,
    );
    const serviceCost = resources.reduce(
      (acc: number, res) => acc + Number(res.quantity || 0) * Number(res.unit_cost || 0),
      0,
    );
    ...
    <div className="h-full flex flex-col bg-white dark:bg-slate-950 font-sans shadow-sm min-h-full">
      ...
      <div className="p-8 pb-4 shrink-0">
        <div className="flex justify-between items-start mb-6">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-medium text-sm">
              ...
              {formData.quote_date
                ? format(new Date(formData.quote_date), 'dd MMMM yyyy', { locale: pl })
                : '-'}
            </div>
          </div>
          <Badge
        ...
        <div className="group relative pl-4 border-l-2 border-blue-500 transition-all duration-300 hover:border-l-4">
          ...
          <h2
            className={`text-2xl font-bold tracking-tight ${!client ? 'text-slate-300' : 'text-slate-900 dark:text-white'}`}
          >
            {clientName}
          </h2>
      ...
      <div className="px-8 mb-4 shrink-0">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div
            className={`rounded-xl p-3 flex flex-col justify-between ${marginValue >= 0 ? 'bg-emerald-50/50 dark:bg-emerald-950/20' : 'bg-red-50/50'}`}
          >
            <span
              className={`text-xs font-medium mb-1 flex items-center gap-1 ${marginValue >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
            >
              <TrendingUp className="h-3 w-3" /> Marża
            </span>
            <div className="flex items-baseline gap-2">
              <span
                className={`text-lg font-bold ${marginValue >= 0 ? 'text-emerald-700' : 'text-red-700'}`}
              >
                {formatMoney(marginValue)}
              </span>
              <span
                className={`text-xs font-semibold ${marginValue >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
              >
                {marginPercent}%
              </span>
            </div>
            ...
            <div className="px-8 space-y-1 pb-6 border-y border-slate-100/50">
              {services.length === 0 ? (
                ...
                services.map((service, idx) => {
                  ...
                  const svcTotal = svcResources.reduce(
                    (acc: number, res) =>
                      acc + Number(res.quantity || 0) * Number(res.unit_price || 0),
                    0,
                  );
                  ...
                  return (
                    <div
                      key={idx}
                      className="group flex justify-between items-center py-3 border-b border-slate-100 last:border-0 hover:bg-white px-3 rounded-lg transition-all"
                    >
