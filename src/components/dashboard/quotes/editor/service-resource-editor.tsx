...
import { Plus, Trash2, GripVertical, AlertCircle, ArrowRight } from 'lucide-react';
...
export function ServiceResourceEditor({ nestIndex, positions }: ServiceResourceEditorProps) {
    ...
    useEffect(() => {
        ...
        setValue(`services.${nestIndex}.subtotal_net`, Number(serviceTotalNet.toFixed(2)), { shouldDirty: true });
        setValue(`services.${nestIndex}.total_net`, Number(serviceTotalNet.toFixed(2)), { shouldDirty: true });
        setValue(`services.${nestIndex}.total_cost`, Number(serviceTotalCost.toFixed(2)), { shouldDirty: true });

    }, [resourcesWatcher, setValue, nestIndex]);

    // Obliczanie podsumowania dla widoku
    const summary = resourcesWatcher?.reduce((acc, curr) => {
    ...
    }, { cost: 0, price: 0, hours: 0 }) || { cost: 0, price: 0, hours: 0 };
    ...
    return (
        <div className="flex flex-col">
            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-slate-50/50 hover:bg-slate-50/50 border-b border-slate-200">
                        ...
                        <TableHead className="w-[200px] text-xs font-semibold text-slate-500 uppercase tracking-tight">Stanowisko</TableHead>
                        <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-tight">Nazwa zadania / Opis</TableHead>
                        <TableHead className="w-[80px] text-xs font-semibold text-slate-500 uppercase tracking-tight text-center">Ilość</TableHead>
                        <TableHead className="w-[120px] text-xs font-semibold text-slate-500 uppercase tracking-tight text-right">Stawka (PLN)</TableHead>
                        <TableHead className="w-[120px] text-xs font-semibold text-slate-500 uppercase tracking-tight text-right">Wartość</TableHead>
                ...
                <TableBody>
                    {fields.map((item, k) => {
                        ...
                        // Alert jeśli sprzedajemy taniej niż kosztujemy (lub bardzo niska marża < 10%)
                        const isLowMargin = (price - cost) / price < 0.1 && price > 0;

                        return (
                            <TableRow key={item.id} className="group hover:bg-blue-50/30 border-b border-slate-100 last:border-0 transition-colors">
                                {/* Grip */}
            ...
            {/* --- PRZYCISK DODAWANIA + PODSUMOWANIE FINANSOWE (FOOTER) --- */}
            <div className="bg-slate-50 border-t border-slate-200 p-3 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                {/* Prawa strona: Podsumowanie Usługi */}
                {fields.length > 0 && (
                    <div className="flex items-center gap-6 text-sm">
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">Koszt</span>
                        ...
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">Marża</span>
                            <div className={`flex items-center gap-1.5 font-bold ${margin >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                <span>{margin.toFixed(2)} zł</span>
                                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${margin >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                        ...
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">Wycena (Netto)</span>
                            <span className="font-bold text-lg text-slate-900">{summary.price.toFixed(2)} zł</span>
                        </div>
                    </div>
                )}
            </div>
